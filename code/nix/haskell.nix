...
############################################################################
CHaP: haskell-nix: nixpkgs-recent: nodePkgs: mithrilPkgs:
  ...
  ({ lib, pkgs, buildProject, ... }: {
    options = {
      ...
      cacheTestFailures = lib.mkOption {
        type = lib.types.bool;
        description =
          "If false, prevent check results from being cached on `nix build`";
        default = true;
      };
  ...
  ({ pkgs, lib, config, buildProject, ... }:
    ...
    let
      ...
      noCacheTestFailuresCookie =
        lib.optionalString (!config.cacheTestFailures) noCacheCookie;
      ...
      localClusterConfigs = config.src
    ...
    in {
      ...
      shell = {
        name = "cardano-wallet-shell${
            lib.optionalString config.profiling "-profiled"
          }";
        packages = ps:
          builtins.attrValues (haskellLib.selectProjectPackages ps);
        tools = {
      ...
      modules = let inherit (config) src coverage profiling;
      in [
        {
        ...
        ({ config, pkgs, ... }:
          let cardanoNodeExes = [ nodePkgs.cardano-cli nodePkgs.cardano-node ];
          in {
            ...
            packages.cardano-wallet-exe.components.exes.cardano-wallet.postInstall =
              optparseCompletionPostInstall + setGitRevPostInstall
              + rewriteLibsPostInstall + stripBinariesPostInstall;
        ...
        (lib.optionalAttrs profiling {
          ...
          packages.cardano-wallet-exe.components.exes.cardano-wallet.enableProfiling =
            true;
          packages.cardano-wallet-benchmarks.components.benchmarks.restore.enableProfiling =
            true;
        ...
        in {
          ...
          packages.cardano-wallet-benchmarks.components.benchmarks.restore =
            fullyStaticOptions;
          packages.cardano-wallet-exe.components.exes.cardano-wallet =
            fullyStaticOptions;
          packages.cardano-wallet-exe.components.exes.cardano-deposit-wallet =
            fullyStaticOptions;
          packages.cardano-wallet-integration.components.tests.integration =
            fullyStaticOptions;
          packages.cardano-wallet-unit.components.tests.unit =
            fullyStaticOptions;
          packages.cardano-wallet-benchmarks.components.benchmarks.db =
            fullyStaticOptions;
          packages.cardano-wallet-launcher.components.tests.unit =
            fullyStaticOptions;
